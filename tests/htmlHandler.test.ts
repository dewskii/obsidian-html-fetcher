...
describe("HtmlHandler", () => {
	...
	describe("fetchToMarkdown", () => {
		it("rejects when requestUrl fails", async () => {
			...
			await expect(handler.fetchToMarkdown("https://mock.sample.foo/", noteFile)).rejects.toThrow(
		...
		it("rejects when Readability does not return article content", async () => {
			...
			await expect(handler.fetchToMarkdown("https://mock.sample.foo/", noteFile)).rejects.toThrow(
				"Readability failed to extract content.",
			);
		});
		...
		it("normalizes app:// URLs in article content", async () => {
			...
			const markdown = await handler.fetchToMarkdown("https://mock.sample.foo/", noteFile);
	...
	describe("fetchToMarkdown image handling", () => {
		it("calls imageHandler.fetchImages by default", async () => {
			...
			await handler.fetchToMarkdown("https://mock.sample.foo/", noteFile);
		...
		it("does not emit markdown image when src is empty", async () => {
			...
			jest.spyOn(ImageHandler.prototype, "fetchImages").mockResolvedValue(undefined);
	...
	describe("coverage todos", () => {
		it.todo(
			"handles extracted article content where body is missing and falls back to empty cleanedHtml",
		);
		it.todo(
			"omits image rule registration when fetchImages is false while still applying table rule",
		);
	});
});
